...
class HippoRAG:
    ...
    def __init__(
        ...
        # 一个 entity 属于哪些 chunk， to chunkid
    ...
    def index(self, docs: List[str]):
        ...
        # 保存映射关系 TripleRawOutput
        self.map_docid_to_entities = ner_results_dict
        self.map_docid_to_triples = triple_results_dict
        ...
        # 对triples的每一个字符串进行清洗， 同时取消映射， 保留下标对应
        chunk_triples = [
    ...
    def add_fact_edges(self, chunk_ids: List[str], chunk_triples: List[List[Triple]]):
        ...
        for chunk_key, triples in tqdm(zip(chunk_ids, chunk_triples)):
            ...
            if chunk_key not in current_graph_nodes:
                ...
                # 以entity node 为 key 记录 chunk_key
                # 一个node 可能对应多个 chunk
    ...
    def add_synonymy_edges(self):
        ...
        for node_key in tqdm(
            ...
            if len(re.sub("[^0-9A-Za-z\u4e00-\u9fff]", "", entity)) > 2:
    ...
    def save_openie_results(self, all_openie_info: List[dict]):
        ...
        sum_phrase_chars = sum(
            [len(str(e)) for chunk in all_openie_info for e in chunk["extracted_entities"]]
        )
        ...
        if len(all_openie_info) > 0:
            ...
            with open(self.openie_results_path, "w") as f:
                json.dump(openie_dict, f, ensure_ascii=False, indent=4)
    ...
    def add_new_edges(self):
        ...
        self.graph.simplify(combine_edges={"weight":"sum"})
    ...
    def rerank_facts(
        ...
        try:
            #LLM rerank 前就只选前 k 个 facts，然后进行rerank
