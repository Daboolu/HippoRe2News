...
class NewsRetrieved:
    question: str
    passages: List[str]
    passages_scores: np.ndarray = None
    ...
    def to_dict(self):
        return {
            ...
            "passages_scores": (
                [round(v, 4) for v in self.passages_scores.tolist()[:5]]
                if self.passages_scores is not None
                else None
            )
        }
...
def reformat_openie_results(
    ...
    # 每个 triples 依然是 List[List[str，str,str]]
...
def flatten_facts(chunk_triples: List[List[Triple]]) -> List[Triple]:
    ...
    for triples in chunk_triples:  \
        # 每个 triples 本身是一个 chunk 内的 List[Triple] 
        # 就是把 Triple 转成 tuple
